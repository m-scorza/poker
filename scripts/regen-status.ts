 ...
 *
 * Writes five sections between <!-- BEGIN:AUTOGEN:<name> --> /
 * <!-- END:AUTOGEN:<name> --> markers:
 *   - verified  — source-derived test counts (replaces the hand-maintained
 *                 "last verified" stamp that kept going stale; no SHA/date so
 *                 it never churns per-commit)
...
function collectTests(): { fileCount: number; relPaths: string[]; totalTests: number } {
  const files: string[] = [];
  ...
  return { fileCount: files.length, relPaths, totalTests };
}

// Source-derivable verification summary. Deliberately contains NO commit SHA or
// date: anything that changes every commit would make `docs:check` fail on the
// next commit. Only counts derived from source content live here, so the stamp
// is enforced yet churn-free.
function verifiedBlock(): string {
  const { fileCount, totalTests } = collectTests();
  return [
...
function testsBlock(): string {
  const { fileCount, relPaths, totalTests } = collectTests();
  return [
    `**Test files:** ${fileCount}`,
    `**\`it\` / \`test\` calls (approximate):** ${totalTests}`,
    '',
