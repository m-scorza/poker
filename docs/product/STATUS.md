...
- **Test files:** 113
- **`it` / `test` calls (approximate):** 1125
...
- `ConfigPage.tsx` — doesn't exist; there is no dedicated settings *page*.
  Settings editing lives in `components/settings/SettingsCard.tsx`, rendered on
  the Data Vault (`/data`) page (ROADMAP Act III-5 settings-UI slice). `Sidebar.tsx`
  still only *displays* the hero name. Consequences:
  - Hero name is editable via the SettingsCard and persists to the IndexedDB
    `settings` table (`saveHeroName`), the row `Layout` re-reads on boot.
  - Strategy profile is selectable (Baseline / Advanced) via the SettingsCard
    and persists through the Zustand persist middleware, so the `advanced`
    profile — thresholds, C-bet logic, leak targets — is now reachable
    (`game_plan` remains the default).
...
7. **Non-standard variants / GGPoker fixture accounting** — ✅ FIXED
   2026-07-22. Cap, Zoom, 6+ Hold'em, and play-money each have dedicated
   assertions in `src/parser/__tests__/fixtureSweep.test.ts`. The four GGPoker
   ZIP archives have an active 53-entry recovery sweep, and all 566 recovered
   hands now parse winner collections and conserve chips, including the
   non-showdown `collected (amount)` summary form.
...
src/__tests__/contentSecurityPolicy.test.ts
src/__tests__/loadingStateHonesty.test.tsx
src/__tests__/numericPresentationBoundary.test.ts
...
src/analysis/__tests__/facingAllIn.test.ts
src/analysis/__tests__/fieldEstimator.test.ts
src/analysis/__tests__/finalTableAnalyzer.test.ts
src/analysis/__tests__/financials.test.ts
src/analysis/__tests__/handRankingWheel.test.ts
...
src/analysis/__tests__/payoutStructures.test.ts
...
src/analysis/__tests__/vs3betAnchors.test.ts
src/components/career/__tests__/BustOutChart.test.tsx
src/components/career/__tests__/CareerStreaksCard.test.tsx
src/components/career/__tests__/DayHourHeatmap.test.tsx
...
src/components/hands/__tests__/HandsTable.test.tsx
...
src/components/settings/__tests__/SettingsCard.test.tsx
...
src/data/__tests__/importDiagnosticsPolicy.test.ts
...
src/data/__tests__/strategyProfiles.test.ts
src/data/ctPacks/__tests__/overlap.test.ts
src/data/ctPacks/__tests__/rangePacks.test.ts
...
src/pages/arena/__tests__/ctRangePacks.test.ts
...
src/utils/__tests__/sessionRows.test.ts
```
<!-- END:AUTOGEN:tests -->
