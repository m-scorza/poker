...
| Third-party curriculum written partnership/license | Red | User reports informal verbal/DM encouragement only; no written terms in repo. |
| Product posture selected | Green | Private/local tool for now. |
| third-party-curriculum-specific IP strategy | Red/changed | User chose to pivot away from third-party-curriculum-specific content. Existing code/docs still contain third-party curriculum/GamePlan/dossier-derived logic and labels. |
| External user validation | Red | Planned but not yet executed. Target: 3 training-community students + 3 independent players. |
...
- Removing or isolating third-party-curriculum-specific references from user-facing copy.
...
- third-party-branded positioning.
- Shareable study-card virality.
- Public exported curriculum-derived PNG/JSON artifacts.
- Deepening third-party-curriculum-specific strategy analysis as a commercial differentiator.
...
2. The product either:
   - has written permission/license terms for third-party-curriculum-derived content, or
   - has a documented plan to remove/isolate third-party-curriculum-specific content from public/commercial flows.
3. `docs/agents/PARTNERSHIP_STATUS.md` reflects the latest truth.

## Current recommended next sprint

Given the user's decision to pivot away from third-party-curriculum-specific content, the likely next sprint is **IP-safe product repositioning**, not new features.

Candidate sprint:

> Make the app safe to demo as a private/local generic poker analyzer by removing third-party curriculum branding/citations from user-facing copy, isolating curriculum-derived internals behind neutral labels, and preserving parser/analysis tests.

Potential acceptance criteria:

- No user-facing text claims third-party curriculum affiliation.
- No pricing/demo page implies rights to third-party curriculum.
