...
| [`agents/`](agents/) | Multi-agent collaboration contracts: handoff log, collab rules, file-area board, and active sprint gate. Historical gate/IP posture docs live under `agents/archive/`. | Hermes, Antigravity, Claude Code, and any other agent picking up work. |
...
| [`research/`](research/) | Pointer only — the research corpus moved to the private `poker-knowledge` repo (2026-07-01 R5 decision). | Strategy discussions; not consumed by code. |
...
2. `agents/TWO_AGENT_BOARD.md` — current gate, assignments, and coordination policy.
...
- **Active command/control docs** live in `agents/` and should stay short. Use
  `agents/TWO_AGENT_BOARD.md` for current routing instead of editing old plans into new plans.
...
- **Source-of-truth order** when docs disagree:
  `code/tests` → `product/STATUS.md` → `agents/TWO_AGENT_BOARD.md` for current routing →
