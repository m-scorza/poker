...
export function DemoPage() {
  return (
    <div className="space-y-10 pb-20">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
          ...
          <div className="grid gap-3 rounded-3xl border border-white/10 bg-black/25 p-5">
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <div><p className="font-bold text-white">For MTT players</p><p className="text-xs text-[var(--color-text-dim)]">Designed around tournament and SNG hand-history formats.</p></div>
            </div>
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <LockKeyhole className="text-[var(--color-accent)]" size={22} />
              <div><p className="font-bold text-white">100% local</p><p className="text-xs text-[var(--color-text-dim)]">Analysis runs in your browser. Histories never leave your machine.</p></div>
            </div>
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <Shield className="text-[var(--color-accent)]" size={22} />
              <div><p className="font-bold text-white">Private review only</p><p className="text-xs text-[var(--color-text-dim)]">Exports are for personal study. No public sharing or distribution.</p></div>
            </div>
          </div>
        </div>
      </section>
        ...
        {FEATURES.map((feature) => (
          <div key={feature.title} className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 shadow-2xl">
            <div className="mb-5 flex items-center gap-3">
              <feature.icon className="text-[var(--color-accent)]" size={22} />
              <h2 className="text-lg font-black uppercase tracking-tight text-white">{feature.title}</h2>
            </div>
            <p className="text-sm font-medium leading-6 text-[var(--color-text-dim)]">{feature.detail}</p>
          </div>
        ))}
      </section>

      <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="mb-6 flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--color-accent)]">
      ...
      <section className="rounded-3xl border border-[var(--color-accent)]/20 bg-[var(--color-accent)]/5 p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[var(--color-accent)]">Private validation</p>
            <p className="mt-2 text-sm font-medium leading-6 text-[var(--color-text-dim)]">
          ...
          <div className="flex gap-3">
            <Link to="/hands" className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-black uppercase tracking-widest text-white hover:border-[var(--color-accent)]/40 hover:text-[var(--color-accent)]">
              Import hands <Download size={16} />
            </Link>
            <Link to="/leaks" className="inline-flex items-center justify-center gap-2 rounded-xl border border-[var(--color-accent)]/40 bg-[var(--color-accent)]/15 px-5 py-3 text-sm font-black uppercase tracking-widest text-[var(--color-accent)] hover:bg-[var(--color-accent)]/25">
