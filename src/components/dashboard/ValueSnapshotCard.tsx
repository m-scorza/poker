...
const verdictStyles: Record<CareerCoachReport['recommendation'], string> = {
  'Move Up Candidate': 'from-emerald-500/25 via-emerald-500/10 to-[var(--ink-2)] border-emerald-400/40 text-emerald-300',
  'Hold Current Stake': 'from-amber-500/25 via-amber-500/10 to-[var(--ink-2)] border-amber-400/40 text-amber-300',
  'Move Down / Rebuild': 'from-rose-500/25 via-rose-500/10 to-[var(--ink-2)] border-rose-400/40 text-rose-300',
  'Need More Sample': 'from-blue-500/25 via-blue-500/10 to-[var(--ink-2)] border-blue-400/40 text-blue-300',
};
...
export function ValueSnapshotCard({ report, leakCount, handCount }: ValueSnapshotCardProps) {
  ...
  return (
    <section className={clsx('relative overflow-hidden rounded-2xl border bg-gradient-to-br p-5 shadow-2xl shadow-black/20 ring-1 ring-white/5', verdictStyles[report.recommendation])}>
      ...
      <div className="pointer-events-none absolute -bottom-24 left-1/4 h-48 w-48 rounded-full bg-[var(--accent)]/10 blur-3xl" />
      ...
      <div className="relative grid gap-5 xl:grid-cols-[1.05fr_1.35fr_auto] xl:items-center">
        ...
        <div className="flex flex-col gap-2 xl:w-52">
          <Link
            to="/career"
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-4 py-3 text-sm font-black uppercase tracking-wider text-black shadow-lg shadow-[var(--accent)]/20 transition hover:-translate-y-0.5 hover:brightness-110"
          >
      ...
      <div className="relative mt-5 rounded-xl border border-white/10 bg-black/20 p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[var(--accent)]">
