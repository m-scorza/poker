...
function MiniMetric({ label, value, helper, icon }: { label: string; value: string; helper: string; icon: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
      <div className="mb-3 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.24em] text-[var(--fg-muted)]">
        {icon}
        {label}
      </div>
      <div className="font-data text-2xl font-black text-white">{value}</div>
      <p className="mt-1 text-[11px] font-medium text-[var(--fg-dim)]">{helper}</p>
    </div>
  );
}
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[var(--color-card-surface)] via-[var(--color-card-gradient-mid)] to-[var(--color-card-gradient-deep)] p-8 shadow-2xl">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.28em] text-[var(--accent)]">
              <Target size={16} />
              Career Coach
            </div>
            ...
            <p className="mt-3 text-sm font-medium leading-6 text-[var(--fg-dim)]">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-black uppercase tracking-widest text-[var(--fg-dim)] transition hover:border-[var(--accent)]/40 hover:text-[var(--accent)]"
              >
                <Download size={13} /> Export private review
              </button>
            </div>
            <p className="mt-3 max-w-xl text-sm font-medium leading-6 text-[var(--fg-dim)]">
          ...
          <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
            <div className="mb-4 flex items-end justify-between gap-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[var(--fg-muted)]">Readiness Score</p>
                <div className="mt-1 font-data text-6xl font-black leading-none text-white">
                  {report.stakeReadinessScore}<span className="text-2xl text-[var(--fg-muted)]">/100</span>
                </div>
              </div>
              <div className="text-right text-xs font-bold uppercase tracking-widest text-[var(--fg-dim)]">
                Confidence<br />
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
              ...
              <div className="space-y-2 text-sm font-semibold text-[var(--fg-dim)]">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
              ...
              {report.topBlocker ? (
                <div>
                  ...
                  <p className="mt-2 text-xs font-medium leading-5 text-[var(--fg-dim)]">{report.topBlocker.detail}</p>
                </div>
              ) : (
          ...
          <div className="rounded-2xl border border-[var(--accent)]/20 bg-[var(--accent)]/5 p-5">
            <div className="mb-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.24em] text-[var(--accent)]">
              <ArrowRight size={14} />
              Next 3 Actions
            </div>
            <ol className="space-y-3">
              {report.nextActions.map((action, index) => (
                <li key={action} className="flex gap-3 text-sm font-semibold leading-6 text-white">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/10 font-data text-xs text-[var(--accent)]">{index + 1}</span>
                  <span>{action}</span>
                </li>
              ))}
            </ol>
