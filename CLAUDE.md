...
The project includes a `/docs/strategy/` directory containing compiled poker theory from the Reg Life coaching program. **Before modifying any analysis logic (leak detection, range compliance, postflop rules, sizing tables), consult the relevant strategy document first.**

#### File Map

| File | Covers | Governs These Code Modules |
|---|---|---|
| `docs/strategy/01-poker-math.md` | EV, pot odds, outs, equity, SPR | `statsCalculator.ts` (EV calculations) |
| `docs/strategy/02-ranges-and-position.md` | RFI by position/stack, 3-bet sizing, range types | `data/ranges.ts`, `rangeChecker.ts` |
| `docs/strategy/03-preflop-strategy.md` | vs RFI reactions, BB defense, blind war, squeeze, 4-bet | `scenarioDetector.ts`, `rangeChecker.ts` |
| `docs/strategy/04-postflop-strategy.md` | C-bet IP/OOP, nuts/range advantage, donk bet, probe | `postflopAnalyzer.ts`, `leakDetector.ts` |
| `docs/strategy/05-icm-and-risk-premium.md` | ICM, bubble factor, risk premium, risk advantage | `icmCalculator.ts` (future), `scenarioDetector.ts` |
| `docs/strategy/06-bounty-tournaments.md` | BPWR, equity drop, PKO adjustments | `bountyAnalyzer.ts` (future) |
| `docs/strategy/07-final-table-play.md` | FT dynamics, fake shove, resteal, facing all-in | `scenarioDetector.ts` (FT mode) |
| `docs/strategy/08-gto-and-exploits.md` | GTO baselines, MDA setup, population exploits | `villainTracker.ts`, `leakDetector.ts` |
| `docs/strategy/09-study-methods-and-tools.md` | Leak finder framework, critical stat targets | `leakDetector.ts`, `statsCalculator.ts` |
| `docs/strategy/10-glossary.md` | Poker term definitions | UI tooltips, help system |
| `docs/strategy/claudecode_index.md` | Concept-to-file mapping | **Read this FIRST for any concept lookup** |

#### Workflow for Claude Code

When asked to modify analysis logic:
1. Read `docs/strategy/claudecode_index.md` to locate the relevant strategy doc
2. Read the specific section of that strategy doc
3. Cross-reference with the current CLAUDE.md rules
4. If the strategy doc provides more nuance than the current hardcoded rule, implement the nuanced version as the "Advanced" strategy profile

#### Source Attribution

Every rule in the codebase should trace back to a source:
- `[GamePlan]` = Reg Life Plano de Jogo SNG (the PDF in project knowledge)
- `[Vol.1]` = Estudar dá Sorte Volume 1
- `[Vol.2]` = Estudar dá Sorte Volume 2
- `[Vol.3]` = Estudar dá Sorte Volume 3
- `[ICM]` = Reg Life ICM eBook
- `[NERD]` = The NERD's Playbook
- `[D#N]` = Dossiê N by @luischiavo

---

### Strategy Profiles (Modular Analysis)

The app supports two analysis modes, selectable by the user:

#### Profile 1: "Game Plan" (Simplified — Current Default)
Based on the Reg Life Plano de Jogo SNG. Rules are binary and easy to follow:
- C-bet IP vs BB: **100% at 33%** on all boards
- SB: raise or fold only (no limp)
- Facing raise (non-BTN/BB): 3-bet or fold only
- BB vs open: never fold suited
- Ranges: exact RFI charts from the game plan PDF

This is the beginner-friendly mode. All current leak detection logic uses this profile.

#### Profile 2: "Advanced Theory" (Nuanced)
Based on the full knowledge base (`/docs/strategy/`). Rules are context-dependent:

**C-bet adjustments by board texture:**
- High-card dry boards (A-7-2, K-Q-x): c-bet 100% at 25-33% `[Vol.2, D#09]`
- Wet broadway boards (K-Q-9 flush draw): c-bet 100% at 50-75% `[D#09]`
- Low connected boards (7-6-5, 8-7-4): check back frequently `[Vol.2, D#10]`
- Paired low boards (9-6-6): check or bet 25% `[D#10]`
- Monotone low boards (8c-6c-3c): check frequently `[Vol.2]`

**C-bet OOP adjustments:**
- Lower frequency than IP
- Prefer larger (polarized) sizing when betting
- Check-raise becomes important defensive tool `[Vol.2]`

**BB defense with ICM awareness:**
- Early game (100% field): never fold suited `[GamePlan]`
- 75% field (RP ~2%): slight tightening `[Vol.3, 05-icm §5]`
- 50% field (RP ~5-8%): fold increases to ~25% `[Vol.3]`
- ITM bubble (RP >10%): fold 40%+, including suited hands `[05-icm §5]`
- Semi-FT (RP >12%): very tight defense `[D#16]`

**3-bet sizing (deep stack additions):**
- 50bb: 3-3.25x IP / 3.5-3.75x OOP `[Vol.2, 02-ranges §5]`
- 100bb: 3.5-4x IP / 4.5-5x OOP `[Vol.2, 02-ranges §5]`

**Leak detection targets (Advanced mode):**

| Stat | Game Plan Target | Advanced Target | Source |
|---|---|---|---|
| C-bet IP | 100% HU | 50-60% overall (texture-dependent) | `[09-study §4]` |
| C-bet leak | <80% | <30% or >80% | `[09-study §4]` |
| WTSD | 25-35% | 25-30% | `[09-study §4]` |
| 3-bet % | - | 7-10% overall | `[09-study §4]` |
| Fold to C-bet | - | 35-45% (leak if <25% or >55%) | `[09-study §4]` |
| VPIP-PFR gap | - | <10 (leak if >15 = too passive) | `[08-gto §3]` |

#### Implementation in Code

```typescript
// src/data/strategyProfiles.ts
...
// Game Plan: always c-bet 33%
const gamePlanCbet: CbetRule = {
  profile: 'game_plan',
  shouldCbet: () => true, // 100% frequency
  recommendedSizing: () => 0.33,
};

// Advanced: texture-dependent
...
Track opponent behavior across sessions for exploitative adjustments.

#### Auto-Collected Stats (per villain)

Every action generates data, no showdown required:
- **VPIP** (by position)
- **PFR** (by position)
- **3-bet %**, Fold to 3-bet %
...
- **Limp %**
- **Open shove frequency**
- Total hands tracked, last seen date

#### Showdown-Derived Data

When villain shows cards at showdown:
- Map shown hand to position + scenario → build actual range profile
- Track: what hands did they open from CO? What did they 3-bet from SB?
- Over time, compare their actual shown range to theoretical ranges

#### Auto-Classification (from MDA framework, `08-gto §3`)

| Archetype | Criteria | Exploit |
|---|---|---|
| **Fish / Recreational** | VPIP − PFR > 15 | Value bet wider, bluff less |
| **Nit** | VPIP < 18, PFR < 14 | Steal wide, fold to aggression |
| **TAG (Tight-Aggressive)** | VPIP 20-28, PFR 18-25, AF > 2 | Respect raises, 3-bet light occasionally |
| **LAG (Loose-Aggressive)** | VPIP 28-40, PFR 24-35, AF > 3 | Call down wider, trap more |
...
Minimum 30 hands for tentative classification, 100+ for confident.

#### Manual Notes

- Free-text notes field per villain
- Tag system: predefined tags like "overfolds turn", "never 3-bets light", "always c-bets"
- Color-coded confidence level (low/medium/high based on sample size)
...
interface VillainProfile {
  ...
  shownHands: ShownHand[]; // hands revealed at showdown
...
### Bounty Tournament Support (Future — Phase 3+)

Based on `docs/strategy/06-bounty-tournaments.md`. Key additions:

#### Tournament Type Detection
Auto-detect PKO/PSKO from hand history (bounty info in PokerStars format).

#### Equity Drop Calculation
When villain is covered, calculate equity drop from bounty:
- Reduces required calling equity by 5-15% depending on bounty size
- Dramatically widens calling ranges

#### Bounty Power (BPWR)
In-game approximation: convert bounty to chip equivalent, add to pot for pot odds calculation.

#### Multi-Way Bounty Pots
When multiple bounties are in play, accumulated discount can justify calling with normally marginal hands.

---

### Corrections to Previous Rules

Based on knowledge base audit, the following rules in this CLAUDE.md are **simplified approximations** that work for the Game Plan profile but should be overridden in Advanced mode:

1. **"C-bet 100% IP vs BB"** — Only on high-card dry boards. Check back on boards where BB has nuts advantage. `[04-postflop §2]`
2. **"WTSD target 25-35%"** — Tighten to 25-30% per leak finder framework. `[09-study §4]`
3. **"3-bet sizing: 30bb+ = 3x IP / 3.5x OOP"** — This is only for 30bb. At 50bb: 3.25x/3.75x. At 100bb: 4x/5x. `[02-ranges §5]`
4. **"BB never fold suited vs open"** — Only in early-mid game with RP < 5%. At bubble+ (RP > 10%), folding suited is correct. `[05-icm §5]`
5. **"C-bet target 60-70%"** — The overall target including multiway is 50-60%. The 100% rule is specifically HU IP on favorable boards. `[09-study §4]`

---

### Updated Implementation Priorities

#### Phase 2.5 — Theory Integration (NEW)
Insert between Phase 2 (Analysis) and Phase 3 (Polish):
1. Implement strategy profile system (Game Plan vs Advanced)
2. Add board texture classifier for context-dependent c-bet analysis
3. Integrate ICM stage detection (early/mid/bubble/FT) for BB defense adjustments
4. Build Villain Tracker with auto-classification
5. Add manual villain notes and tags

#### Phase 4 — Bounty & Advanced (NEW)
1. Bounty tournament detection and equity drop calculation
2. BPWR calculator
3. FT-specific analysis (fake shove detection, resteal spots)
4. Squeeze play detection and analysis
