...
*   [docs/reports/README.md](./docs/reports/README.md) — active health reports
    (`status: open`/`in_progress`) are surfaced each session by the SessionStart
    hook in `.claude/settings.json`; a README index (autogenerated,
    `docs:check`-enforced) tracks every report. When a report is done, close its
    `## Open items`, set `status: resolved`/`superseded`, and `git mv` it into
    `docs/reports/archive/` — don't let recommendations pile up.
...
## Key Dependencies (verified against `package.json` 2026-07-15)
