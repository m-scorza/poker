...
| A17 | Parser surface (`pokerstars.ts`, `ggpoker.ts`, `openHandHistory.ts`) | ~~Side pots are silently ignored.~~ **NOT-A-BUG** (verified 2026-05-18, regression test in `pokerstars.test.ts`). PokerStars emits the *grand total first*: `Total pot 37070 Main pot 7750. Side pot 29320. \| Rake 0`. The existing regex `/Total pot \$?([\d.]+)/` captures `37070`, so `hand.totalPot` is correct on multi-way all-ins. Run-it-twice and disconnects remain unmodeled (separate concerns, not exercised by current fixtures). | — | NOT_A_BUG |
...
| G3 | `src/analysis/bountyAnalyzer.ts` | ~~Hardcoded 1 500 starting stack~~ — **✓ RESOLVED (#231, verified 2026-08-07).** No magic constant remains. The starting stack is derived per tournament from the max `chipsBefore` across the earliest-level hands (`bountyAnalyzer.ts:81–89`) and threaded in as `startingStack`; the documented fallback is `bigBlind * 75`, and the degenerate `bigBlind === 0` case yields 0 rather than fabricating a chip value. No owner answer needed — this was reclassified from `ASK_USER` on evidence. | ✓ |
...
| G7 | Side pots | ~~Not parsed~~ — see revised A17. **NOT-A-BUG.** `Total pot` line in SUMMARY emits the grand total before the "Main pot $X. Side pot $Y." breakdown, so `RE_TOTAL_POT` captures the sum. Equity-vs-pot-size math is unbiased. Future enhancement: expose main/side breakdown separately for downstream pot-odds-per-pot analysis (not currently needed). | ✓ clean |
...
| ~~H5~~ | ~~Add side-pot parsing to `pokerstars.ts` `RE_TOTAL_POT`~~ | **NOT-A-BUG** — verified 2026-05-18. PokerStars summary line emits the grand total *first*: `Total pot 37070 Main pot 7750. Side pot 29320.`. The existing regex `/Total pot \$?([\d.]+)/` captures `37070` (the sum), not the main pot. Regression test added in `pokerstars.test.ts` covering single-side-pot and multi-side-pot formats. The underlying *enhancement* — exposing `mainPot`/`sidePot` separately for downstream analysis — remains a possible future feature, but `hand.totalPot` is correct today. | — | A17, G7 |
